...
export const CardDetailModal: React.FC = () => {
  ...
  const handlePostComment = async (e: React.FormEvent) => {
    ...
    await addComment(selectedCardId, commentText.trim());
    setCommentText('');
    fetchDetails();
  };
    ...
    <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white dark:bg-neutral-900 rounded-2xl w-full max-w-3xl max-h-[90vh] shadow-2xl border border-neutral-200 dark:border-neutral-800 flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-150">
        ...
        <div className="flex-1 overflow-y-auto p-6 grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="space-y-4">
                  ...
                  <form onSubmit={handlePostComment} className="flex gap-2">
                    <input
                      ...
                      className="flex-1 text-xs px-3 py-2 rounded-lg border border-neutral-200 dark:border-neutral-800 bg-neutral-50 dark:bg-neutral-950 text-neutral-900 dark:text-neutral-100 focus:outline-none"
                    />
                    <button
                      type="submit"
                      className="p-2 bg-neutral-900 dark:bg-white text-white dark:text-neutral-900 rounded-lg hover:opacity-90 transition-opacity shrink-0"
                    >
                      <Send size={14} />
                    </button>
                  </form>
                  ...
                  <div className="space-y-3 max-h-56 overflow-y-auto">
                    {cardDetails.comments?.map((c: any) => (
                      <div key={c.id} className="bg-neutral-50 dark:bg-neutral-800/40 p-3 rounded-xl text-xs space-y-1">
                        <div className="flex items-center justify-between text-[11px] text-neutral-400">
                          <span className="font-semibold text-neutral-700 dark:text-neutral-300">{c.user?.name}</span>
          ...
          {/* Sidebar Properties */}
          <div className="space-y-5 bg-neutral-50 dark:bg-neutral-950/60 p-4 rounded-xl border border-neutral-200/80 dark:border-neutral-800/80">
            {/* Priority */}
            <div>
